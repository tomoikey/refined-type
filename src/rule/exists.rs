...
pub type Exists<RULE> = Refined<ExistsRule<RULE>>;

/// Rule where at least one data in the collection satisfies the condition
pub type ExistsRule<RULE> = Not<ForAllRule<Not<RULE>>>;
...
mod tests {
    ...
    fn exists_1() -> anyhow::Result<()> {
        ...
        let exists: Exists<NonEmptyStringRule> = Exists::new(value.clone())?;
    ...
    fn exists_2() -> anyhow::Result<()> {
        ...
        let exists_result = Exists::<NonEmptyStringRule>::new(value.clone());
